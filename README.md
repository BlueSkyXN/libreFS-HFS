...
浏览器 HTML 请求的 token 验证成功后，服务会设置 `HttpOnly` cookie 并跳转到不带 token 的 `/_ops/`；后续浏览器访问 `/_ops/health`、`/_ops/system` 等不需要继续把 token 放在 URL 里。脚本/API 请求不接受 query token 鉴权，应使用 header 或 bearer token；不要把带 `?token=` 的链接写入文档、截图或分享链接。
