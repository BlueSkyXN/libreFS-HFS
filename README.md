...
| 只读 Ops | 可用 / 需 token | `/_ops/` 提供 health、system、config、version、metrics；默认使用 `OPS_TOKEN` 保护，只返回非敏感摘要。 |
| Admin 管理面 | 默认关闭 | `/_admin/` 默认 `ADMIN_ENABLED=false` 返回 404；开启后只提供极小白名单 action。 |
...
| `/_ops/` | `127.0.0.1:8081` | 只读诊断面；需要 `OPS_TOKEN`。 |
...
| Secret | `ADMIN_TOKEN` | 仅开启 admin 时 | `/_admin/` 独立 token；只有 `ADMIN_ENABLED=true` 时需要。 |
