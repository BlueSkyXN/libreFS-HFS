...
| 启动脚本 | `start.sh` | 校验必需 Secrets，设置公开 URL 环境变量，启动 libreFS、ops-service、admin-service 和 Nginx，处理退出。 |
| 反向代理 | `nginx.conf` | 把公开 `7860` 流量分发到 ops、admin、Console 和 S3 API。 |
| 只读诊断 | `ops_service.py` | 提供 `/_ops/health`、system、config、version 和 metrics。 |
| 管理面 | `admin_service.py` | 默认关闭；开启后提供 status、action catalog、run-health-checks 和 reload-nginx。 |
...
flowchart TD
  ...
  J --> K["Copy librefs, ops/admin services, nginx.conf, start.sh"]
  K --> L["Final image"]
```
...
| `/_ops/` | `http://127.0.0.1:8081/` | 只读诊断入口，需要 `OPS_TOKEN`。 |
