...
| 默认关闭管理面 | `admin_service.py` 提供默认关闭的 status/actions/run-health-checks/reload-nginx 白名单。 | `admin_service.py` |
...
| P1 | Ops/Admin live 验收 | 本地静态检查能验证路由和服务语法，但不能证明 HF runtime 已接管新镜像。 | 发布到 HF 后检查 `/_ops/health`、`/_admin/` 默认 404、runtime logs 和 Space `runtime.raw.sha`。 |
...
- 未执行需要 root 凭证的 S3 写入/读取 smoke test；本轮只完成脚本和语法验证。
