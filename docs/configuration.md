...
`?token=` 只适合首次网页登录或临时浏览器调试，不建议写进文档、脚本、截图或分享链接。容器内 Nginx access log 不记录 query string，ops-service 日志会 redact `token=` 值，并且 ops 响应带 `Referrer-Policy: no-referrer`；脚本和自动化应优先使用 `X-Ops-Token` 或 `Authorization: Bearer <token>`。
