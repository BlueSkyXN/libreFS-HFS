...
require_pattern hfs-dev.toml '^space_visibility = "protected"$' 'canonical Space visibility must be Protected'
require_pattern hfs-dev.toml '^bucket_visibility = "private"$' 'canonical bucket visibility must be Private'
...
require_pattern hfs-dev.candidate.toml '^space_visibility = "protected"$' 'candidate Space visibility must be Protected'
require_pattern hfs-dev.candidate.toml '^bucket_visibility = "private"$' 'candidate bucket visibility must be Private'
...
require_pattern scripts/sample-hf-bucket-storage.sh '^EXPECTED_HF_HUB_VERSION="1\.25\.1"$' 'storage sampler must pin huggingface_hub 1.25.1'
require_pattern scripts/sample-hf-bucket-storage.sh '^EXPECTED_CLICK_VERSION="8\.4\.2"$' 'storage sampler must pin click 8.4.2'
...
require_pattern .github/workflows/deploy-hf-space.yml 'HF_CLI_VERSION: "1\.25\.1"' 'workflow must pin Protected-compatible huggingface_hub 1.25.1'
require_pattern .github/workflows/deploy-hf-space.yml 'HF_CLI_CLICK_VERSION: "8\.4\.2"' 'workflow must pin the module CLI click runtime'
...
require_pattern .github/workflows/deploy-hf-space.yml 'repos settings --help.*grep -- --protected' 'workflow must verify Protected visibility support'
